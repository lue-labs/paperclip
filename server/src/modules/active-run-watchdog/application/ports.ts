...
export interface WatchdogRunReader {
  ...
  /**
   * Fork: batched forms of the two reads above, keyed by run id, so list
   * endpoints (live-run lists) issue a bounded number of queries per page
   * instead of two per run. Runs absent from the map have no decision /
   * evaluation.
   */
  findLatestDecisions(companyId: string, runIds: string[], now: Date): Promise<Map<string, FindLatestDecisionResult>>;
  findOpenStaleRunEvaluations(companyId: string, runIds: string[]): Promise<Map<string, EvaluationIssueSnapshot>>;
