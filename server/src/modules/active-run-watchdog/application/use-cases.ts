...
function summarizeRunOutputSilence(
  deps: Pick<BuildRunOutputSilenceDeps, "suspicionThresholdMs" | "criticalThresholdMs">,
  run: BuildRunOutputSilenceRunInput,
  now: Date,
  decisionState: { dismissedFalsePositive: boolean; quietUntilDecision: { snoozedUntil: Date } | null },
  evaluation: EvaluationIssueSnapshot | null,
): RunOutputSilenceSummary {
...
export function createBuildRunOutputSilence(deps: BuildRunOutputSilenceDeps) {
  return async function buildRunOutputSilence(
    ...
    return summarizeRunOutputSilence(deps, run, now, decisionState, evaluation);
  };
}

/**
 * Fork: batched companion to `buildRunOutputSilence` for list endpoints —
 * two queries per page instead of two per run.
 */
export function createBuildRunOutputSilenceMap(deps: BuildRunOutputSilenceDeps) {
  return async function buildRunOutputSilenceMap(
    companyId: string,
    runs: BuildRunOutputSilenceRunInput[],
    now: Date,
  ): Promise<Map<string, RunOutputSilenceSummary>> {
    const runIds = runs.map((run) => run.id);
    const [decisions, evaluations] = await Promise.all([
      deps.reader.findLatestDecisions(companyId, runIds, now),
      deps.reader.findOpenStaleRunEvaluations(companyId, runIds),
    ]);
    const result = new Map<string, RunOutputSilenceSummary>();
    for (const run of runs) {
      result.set(
        run.id,
        summarizeRunOutputSilence(
          deps,
          run,
          now,
          decisions.get(run.id) ?? { dismissedFalsePositive: false, quietUntilDecision: null },
