...
export function recoveryService(
  ...
  /**
   * Fork: non-runnable agent states are holds, not transient failures:
   * enqueueWakeup would refuse (budget hard-stop, paused/not-invokable agent)
   * or skip (heartbeat.wakeOnDemand disabled) anyway, and re-attempting on
   * every recovery sweep persisted a skipped agent_wakeup_requests row each
   * time — an unbounded wake-request storm while the state lasted. Backstops
   * re-scan, so held wakes resume on the first sweep after the agent state
   * changes.
   */
  async function getAgentWakeHold(
    ...
    if (!isHeartbeatWakeOnDemandEnabled(agent)) return "wake_on_demand_disabled";
  ...
  async function enqueueStrandedIssueRecovery(input: {
    ...
    {
      // Fork: wake holds (see getAgentWakeHold).
  ...
  // Fork: batched companion for list endpoints.
  async function buildRunOutputSilenceMap(
    companyId: string,
    runs: Parameters<typeof buildRunOutputSilence>[0][],
    now = new Date(),
  ): Promise<Map<string, RunOutputSilenceSummary>> {
    return watchdog.buildRunOutputSilenceMap(companyId, runs, now);
  }
  ...
  // Fork (#28): a routine-execution issue whose routine has since completed a
  // newer run with the same dispatch fingerprint is stale; cancel it instead
  // of recovering it.
  ...
  async function reconcileResolvedDependencyWakeBackstop(
    ...
    for (const [
      ...
      for (const candidate of companyCandidates) {
        ...
        // Fork: any non-runnable agent state (paused/not-invokable,
        // wakeOnDemand disabled, budget hard-stop) is a hold: skip before
        // enqueueWakeup so repeated sweeps do not persist skipped rows.
