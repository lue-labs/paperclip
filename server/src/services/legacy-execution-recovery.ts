...
import type { RunClaim } from "./run-ownership-store.js";
...
export async function terminalizeLegacyExecution(input: {
  ...
  fromStatuses?: string[];
  /**
   * Fork (active-active): fence the terminal write against the caller's
   * run ownership. When set, the write only lands if the row still carries
   * exactly this (ownerToken, fence) pair. See run-ownership-store.ts.
   */
  claim?: RunClaim | null;
}) {
  const { db, run, status, patch, claim } = input;
  // A terminal status always releases ownership in the same statement — a
  // finished run has nothing left to fence, and a stale owner_token would
  // make it look claimable-by-CAS forever.
  const releasePatch = TERMINAL_RUN_STATUSES.has(status)
          ...
          ...(claim
            ? [
                ...
                claim.fence === null
                  ...
                  : eq(heartbeatRuns.fence, claim.fence),
              ]
            : []),
        ),
      )
