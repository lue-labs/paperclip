...
export class PaperclipControlPlanePort implements ControlPlanePort {
  ...
  async appendEvent(value: NativeRunEvent | PrpEvent) {
    ...
    // Fork: appendHeartbeatRunEvent can report `rejected` when a fenced claim
    // is stale. The control-plane port appends unfenced, so this is
    // unreachable here; surface it as an error rather than widen the contract.
    if (persisted.disposition === "rejected") {
      throw new Error("native_event_ownership_rejected");
    }
