...
describeEmbeddedPostgres("heartbeat resolved dependency wake reconciliation", () => {
  ...
  it("holds resolved dependency wakes for paused agents instead of retrying every sweep", async () => {
    ...
    for (const result of [first, second]) {
      ...
      expect(result.wakeHoldSkipped).toBe(1);
    ...
    const afterResume = await heartbeat.reconcileResolvedDependencyWakes();
    expect(afterResume.healed).toBe(1);
    expect(afterResume.issueIds).toEqual([blockedIssueId]);
  });
  ...
  it("holds resolved dependency wakes for wakeOnDemand-disabled agents instead of retrying every sweep", async () => {
    ...
    await db
      ...
      .where(eq(agents.id, agentId));
    const afterEnable = await heartbeat.reconcileResolvedDependencyWakes();
    expect(afterEnable.healed).toBe(1);
    expect(afterEnable.issueIds).toEqual([blockedIssueId]);
  });
  ...
  it("holds resolved dependency wakes for budget-stopped agents instead of retrying every sweep", async () => {
    ...
    const afterClear = await heartbeat.reconcileResolvedDependencyWakes();
    expect(afterClear.healed).toBe(1);
    expect(afterClear.issueIds).toEqual([blockedIssueId]);
  });
