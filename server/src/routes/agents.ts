...
import { activityLog, agentWakeupRequests, agents as agentsTable, chatConversations, companies, heartbeatRuns, issues as issuesTable, projects as projectsTable } from "@paperclipai/db";
...
export function agentRoutes(
  ...
  async function buildSkippedWakeupResponse(
    ...
    // Fork (#102): surface the durable skip reason instead of a generic message.
  ...
  // Fork: provider-fallback ladder entries carry their own adapterConfig.
  // They must honour the same instruction/host-command mutation guards and
  // credential mediation as the primary adapterConfig.
  ...
  async function normalizeRuntimeConfigAdapterConfigsForPersistence<T extends Record<string, unknown>>(
    companyId: string,
    adapterType: string,
    runtimeConfig: T,
    ...
    if (fallbackEntries.length === 0) return runtimeConfig;
    ...
    const providerFallbacks = Array.isArray(runtimeConfig[fallbackField])
      ? [...(runtimeConfig[fallbackField] as unknown[])]
      : [];
    ...
    return { ...runtimeConfig, [fallbackField]: providerFallbacks };
  }
  ...
  router.post("/companies/:companyId/agent-hires", validate(createAgentHireSchema), async (req, res) => {
    ...
    const normalizedRuntimeConfig = await normalizeRuntimeConfigAdapterConfigsForPersistence(
      companyId,
      hireInput.adapterType,
      await normalizeCreatedAgentRuntimeConfig(req, companyId, hireInput.adapterType, normalizedAdapterConfig, hireInput.runtimeConfig),
      normalizedAdapterConfig,
    );
  ...
  router.post("/companies/:companyId/agents", validate(createAgentSchema), async (req, res) => {
    ...
    const normalizedRuntimeConfig = await normalizeRuntimeConfigAdapterConfigsForPersistence(
      companyId,
      createInput.adapterType,
      await normalizeCreatedAgentRuntimeConfig(req, companyId, createInput.adapterType, normalizedAdapterConfig, createInput.runtimeConfig),
      normalizedAdapterConfig,
    );
  ...
  type WakeupRouteOpts = {
    ...
    skippedResponse: (agent: NonNullable<Awaited<ReturnType<typeof svc.getById>>>, payload: Record<string, unknown> | null, idempotencyKey: string) => unknown | Promise<unknown>;
  };
  const handleWakeupRoute = async (
    ...
    // Fork (#102): always mint a key so the skipped-wakeup row is retrievable.
    ...
    if (!run) {
      res.status(202).json(await opts.skippedResponse(agent, wakePayload, idempotencyKey));
      return;
    }
  ...
  router.post("/agents/:id/wakeup", validate(wakeAgentSchema), async (req, res) => {
    await handleWakeupRoute(req, res, {
      ...
      skippedResponse: (agent, payload, idempotencyKey) => buildSkippedWakeupResponse(agent, payload, idempotencyKey),
    });
  });
  ...
  router.get("/companies/:companyId/live-runs", async (req, res) => {
    ...
    if (targetRunCount > 0 && liveRuns.length < targetRunCount) {
      ...
      const [projections, outputSilenceByRunId] = await Promise.all([
        executionProjectionsForRuns(db, companyId, rows.map(run => run.id)),
        heartbeat.buildRunOutputSilenceMap(companyId, rows),
      ]);
      res.json(await runRedactions.redactForRuns(companyId, rows.map((run) => ({
    ...
    const [projections, outputSilenceByRunId] = await Promise.all([
      executionProjectionsForRuns(db, companyId, liveRuns.map(run => run.id)),
      heartbeat.buildRunOutputSilenceMap(companyId, liveRuns),
    ]);
    res.json(await runRedactions.redactForRuns(companyId, liveRuns.map((run) => ({
      ...heartbeat.decorateActiveRunStatus(run),
      execution: projections.get(run.id) ?? null,
      outputSilence: outputSilenceByRunId.get(run.id),
    }))));
  ...
  router.get("/issues/:issueId/live-runs", async (req, res) => {
    ...
    const [projections, outputSilenceByRunId] = await Promise.all([
      executionProjectionsForRuns(db, issue.companyId, liveRuns.map(run => run.id)),
      heartbeat.buildRunOutputSilenceMap(issue.companyId, runsWithCompanyId),
    ]);
