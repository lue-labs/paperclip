...
const refreshExternalObjectsSchema = z
  .object({
    ...
    // Fork (#92): bypass nextRefreshAt for an explicit operator refresh.
...
// Fork (#131): comments report the status transition they caused, and
// `reopen: false` opts out of the implicit reopen entirely.
...
export function issueRoutes(
  ...
  router.patch(
    ...
    async (req, res) => {
      ...
      // Fork: a cancelled issue cancels every queued/running run and pending
      // wakeup bound to it, not just the single resolved active run. The
      // helper only touches cancellable statuses, so a run already stopped
      // via `interruptedRunId` above is left alone.
      ...
      if (shouldCancelActiveRunForCancelledStatus) {
        try {
          ...
          logger.warn(
            ...
            "failed to cancel invocations for cancelled issue",
          );
          await logActivity(db, {
            companyId: existing.companyId,
            actorType: actor.actorType,
