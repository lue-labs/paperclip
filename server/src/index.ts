...
async function startServerWithDatabaseTeardown(
  ...
  if (heartbeat) {
    ...
    // into a dead "running" row during startup recovery. The HTTP listener is
    // already bound above (upstream #12845) with health reporting phase
    // "recovering", so this no longer blocks bind (fork #138 concern).
